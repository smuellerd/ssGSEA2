...
run_ssGSEA2 <- function(input.ds,
  ...
  if(export.signat.gct){
    ...
    for( sig.name in rownames(Signature.set.overlap)) {
      ...
      write_gct(gct.tmp, ofile=sub('.*/', 'signature_gct/', fn), appenddim = T)
...
#' Chop string
#' 
#' Given a string and a number of characters, the function chops the string to the
#' specified number of characters and adds '...' to the end.
#'
#' @param string character
#' @param nChar numeric
#' @param add.dots logical
#'
#' @return string
#' @keywords internal
#'
chopString <- function(string, nChar=10, add.dots=TRUE){
...
#' @param alpha integer
#' @param maxColorValue integer
#'
#' @return vector of results (one per input color)
#' @keywords internal
#'
